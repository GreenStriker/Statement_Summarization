...
for x in range(get_pdf_length):
    ...
    with open(directory_path + "/Text file/document.txt", "r", encoding="utf-8") as f:
        for line in f:
            if ((re.match(r"([A-Za-z0-9_]{1,2}/[A-Za-z0-9_]{1,3}/[\d]{1,4})", line)) or (
                 re.match(r"([A-Za-z0-9_]{1,2}/[A-Za-z0-9_]{1,3}[\d]{1,4})", line)) or (
                 re.match(r"([A-Za-z0-9_]{1,2}[A-Za-z0-9_]{1,3}/[\d]{1,4})", line))):
